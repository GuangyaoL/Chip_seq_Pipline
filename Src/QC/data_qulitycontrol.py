...
from config import work_base
try:
    conf = work_base.get_ymal_load(os.path.join(basedir, 'config', 'conf.YAML'))
except:
    work_base.logger('the config maybe wrong!!!')
    # print('the config maybe wrong!!!')
    sys.exit(1)
def qulity_control(fastp, failed_out,splitlines, r1, r2, qulity, Qpercentlimit,o1=None, o2=None,
                   ...
                   adapterfa=conf['Info']['adapter'], fastfa=os.path.join(basedir,'config','adaper.fa')):
    '''
    ...
    try:
        ...
        work_base.logger('ReadFileGet').error('R1 name {} maybe error! please check it!'.format(os.path.basename(r1)))
        sys.exit(1)
    ...
    if not re.match('^[a-zA-z0-9]',samplename):
        work_base.logger('SampleNameGet').error('the sample name {} is out of rules,please used letters and numbers start!'.format(
            ppsamplename
        ))
        sys.exit(1)
    with open(samplename+'.qc.sh','w')as fw:
        fw.write(conf['Info']['shell_heaader'])
        fw.write('date\n')
        if failed_out:
            ...
            fw.write(
                '{fastp} -i {r1} -I {r2} -o {o1} -O {o2} -S {splitlines} -q {qulity} -u {qsp} -j {j} -h {h} -y --detect_adapter_for_pe --adapter_sequence {r1f} --adapter_sequence_r2 {r2f} --adapter_fasta {adpt}\n'.format(
                    ...
                    qsp=Qpercentlimit,
                    h=html, j=json, adpt=fastfa, r1f=conf['Info']['adapter']['adapter_sequence_r1'],
                    r2f=conf['Info']['adapter']['adapter_sequence_r2']
                ))
        fw.write('date\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--software', '-p',help='the fastp software path,default get in config',
                        default=conf['software']['QC']['fastp'])
    ...
    if args.unqualified_percent_limit not in range(1,101):
        work_base.logger('UnQualified_Percent').error('the unqualified_percent_limit are the percent ,so you should input less 100')
        sys.exit(1)
    #(fastp, failed_out,splitlines, r1, r2, qulity, Qpercentlimit,
    qulity_control(args.software, args.failed_out,400000,args.read1,args.read2,args.qulity,args.unqualified_percent_limit)
